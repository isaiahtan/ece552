...
/*	for (i = 0; i < RESERV_FP_SIZE; i++){
			if (reservFP[i] == NULL)
				printf("RS FP %d is free\n", i);
			else
				printf("RS FP %d has %d\n", i, reservFP[i]->index);
		}
		for (i = 0; i < RESERV_INT_SIZE; i++){
			if (reservINT[i] == NULL)
				printf("RS INT %d is free\n", i);
			else
				printf("RS INT %d has %d\n", i, reservINT[i]->index);

		}
	for (i = 0; i < FU_FP_SIZE; i++){
			if (fuFP[i] == NULL)
				printf("FU FP %d is free\n", i);
			else
				printf("FU FP %d has %d\n", i, fuFP[i]->index);

		}
		for (i = 0; i < FU_INT_SIZE; i++){
			if (fuINT[i] == NULL)
				printf("FU INT %d is free\n", i);
			else
				printf("FU INT %d has %d\n", i, fuINT[i]->index);
  ...
  for (i = 0; i < FU_INT_SIZE; i++) {
	  if (fuINT[i] == NULL) {
		  ...
		  for (j = 0; j < RESERV_INT_SIZE; j++) {
			  ...
			  if (instr->tom_execute_cycle > 0) continue; // Executed already

			  // Check if instruction is ready to execute.
			  ready = 1;
			  for (k = 0; k < 3; k++) {
				  if (instr->Q[k] != NULL) {
					  //printf("RS INT %d (%d) is dependent on %d, %d\n", j, instr->index, instr->Q[k]->index, k);
  ...
  for (i = 0; i < FU_FP_SIZE; i++) {
	  if (fuFP[i] == NULL) {
		  ...
		  for (j = 0; j < RESERV_FP_SIZE; j++) {
			  instr = reservFP[j];
			  if (instr == NULL) continue;
			  if (instr->tom_execute_cycle > 0) continue; // Executed.

			  // Check if instruction is ready to execute.
			  ready = 1;
			  for (k = 0; k < 3; k++) {
				  if (instr->Q[k] != NULL) {
					//printf("RS FP %d (%d) is dependent on %d, %d\n", j, instr->index, instr->Q[k]->index, k);
					  // One of the input is not ready.
					  ready = 0;
					  break;
				  }
...
counter_t runTomasulo(instruction_trace_t* trace)
{
  ...
  while (true) {
	...
	cycle++;
        if (is_simulation_done(sim_insn_count) || cycle >= 10000) {
