...
static counter_t reg_ready_lab1_p1[MD_TOTAL_REGS];
static counter_t reg_ready_lab1_p2[MD_TOTAL_REGS];
...
static counter_t sim_num_RAW_hazard_q1_1stall;
static counter_t sim_num_RAW_hazard_q1_2stall;
static counter_t sim_num_RAW_hazard_q2_1stall;
static counter_t sim_num_RAW_hazard_q2_2stall;
...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q1",
		   ...
		   "1 + sim_num_RAW_hazard_q1_1stall/sim_num_insn + 2*sim_num_RAW_hazard_q1_2stall/sim_num_insn", NULL);

  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q2",
		   "CPI from RAW hazard (q2)",
		   "1 + sim_num_RAW_hazard_q2_1stall/sim_num_insn + 2*sim_num_RAW_hazard_q2_2stall/sim_num_insn", NULL);
...
#include "machine.def"
	default:
      ...
      //if operation writes a register in any way, track 2 stalls
      // all integer operations will use the register file
      {
	      int i;
	      for (i = 0; i < 3; i++) { // run for a 3 possible input registers
		      // Prob 1
		      if (r_in[i] != DNA && reg_ready_lab1_p1 [r_in [i]] > sim_num_insn) {
			      // if instruction uses a register as input, and the register is not ready
			      // not ready is defined as the insn count is not up to the expected number
			      if ( reg_ready_lab1_p1 [r_in [i]] == sim_num_insn + 2 ) // recorded that the incoming insn needs to be stalled for 2 cycles
				      sim_num_RAW_hazard_q1_2stall++;
			      else if ( reg_ready_lab1_p1 [r_in [i]] == sim_num_insn + 1) // recorded that the incoming insn needs to be stalled for 1 cycle
				      sim_num_RAW_hazard_q1_1stall++;

			      // Record total number of hazards
			      sim_num_RAW_hazard_q1++;	

			      // "Reset" reg_ready so that we don't double count 2-cycle-stall hazards
			      // as another 1-cycle-stall hazard in the next cycle.
			      reg_ready_lab1_p1 [r_in[i]] = sim_num_insn;

			      break;
		      }
	      }
      }
      { // separate for loops used for each question. break will mess up the for loop, so keep stat counters isolated
	      int i;
	      for (i = 0; i < 3; i++) {
		      // Prob 2
		      if (r_in[i] != DNA && reg_ready_lab1_p2 [r_in [i]] > sim_num_insn){
			      // if instruction uses a register as input, and the register is not ready
			      // not ready is defined as the insn count is not up to the expected number
			      if ( reg_ready_lab1_p2 [r_in [i]] == sim_num_insn + 2 ) // record that the incoming insn needs to be stalled for 2 cycles
				      sim_num_RAW_hazard_q2_2stall++;
			      else if ( reg_ready_lab1_p2 [r_in [i]] == sim_num_insn + 1)// record that the incoming insn needs to be stalled for 1 cycle
			      ...
			      // "Reset" reg_ready so that we don't double count 2-cycle-stall hazards
			      // as another 1-cycle-stall hazard in the next cycle.
			      reg_ready_lab1_p2 [r_in[i]] = sim_num_insn;
			      break;
		      }
      ...
      int i;
      for (i = 0; i < 2; i++){
	      if (r_out[i] != DNA){ // if current insn uses an output register
		      // this register will be used as output. set the expected "ready" cycle number
		      // can use register on writeback stage. assuming that the write to regfile is on first half of clock cycle, 
		      //	and read is on second half. then decode can read from regfile on 3rd cycle.
		      reg_ready_lab1_p1[r_out[i]] = sim_num_insn + 3; // 2 stalls, ready on third. Prob 1

		      if (MD_OP_FLAGS(op) & F_LOAD){ // for loading from memory, make sure register is only usable after the memory stage
			      // can forward from MW latch to DX1 latch
			      reg_ready_lab1_p2[r_out[i]] = sim_num_insn + 3; // 2 stalls, ready on third. Prob 2
		      } 
		      else { // since 2 stage execute, always stall one cycle, if not loading from memory
			      reg_ready_lab1_p2[r_out[i]] = sim_num_insn + 2; // 1 stalls, ready on second. Prob 2
		      }
	      }
      }
